...
static __attribute__((section(".fast_ram"))) USB_MEM_ALIGNX uint8_t g_rndis_rx_buffer[CONFIG_USBHOST_RNDIS_ETH_MAX_RX_SIZE];
static __attribute__((section(".fast_ram"))) USB_MEM_ALIGNX uint8_t g_rndis_tx_buffer[CONFIG_USBHOST_RNDIS_ETH_MAX_TX_SIZE];
...
find_class:
    ...
    g_rndis_rx_length = 0;
    while (1) {
        usbh_bulk_urb_fill(&g_rndis_class.bulkin_urb, g_rndis_class.hport, g_rndis_class.bulkin, (uint8_t *)core_local_mem_to_sys_address(0, (uint32_t)&g_rndis_rx_buffer[g_rndis_rx_length]), transfer_size, USB_OSAL_WAITING_FOREVER, NULL, NULL);
...
__attribute__((section(".fast"))) void usbh_rndis_tcp_done()
{
    uint32_t len;

    if (g_rndis_class.tx_offset == 0) {
        return;
    }

    len = g_rndis_class.tx_offset;
    ...
    usbh_bulk_urb_fill(&g_rndis_class.bulkout_urb, g_rndis_class.hport, g_rndis_class.bulkout, (uint8_t *)core_local_mem_to_sys_address(0, (uint32_t)g_rndis_tx_buffer), len, USB_OSAL_WAITING_FOREVER, NULL, NULL);
    usbh_submit_urb(&g_rndis_class.bulkout_urb);
    g_rndis_class.tx_offset = 0;
}

extern uint8_t write_finished;

__attribute__((section(".fast"))) int usbh_rndis_eth_output(uint8_t *buf, uint32_t buflen)
{
    uint8_t *buffer;
    ...
    if ((g_rndis_class.tx_offset + 1558) > g_rndis_class.max_transfer_size) {
        usbh_rndis_tcp_done();
    }

    hdr = (rndis_data_packet_t *)(g_rndis_tx_buffer + g_rndis_class.tx_offset);
    ...
    buffer = (uint8_t *)(g_rndis_tx_buffer + g_rndis_class.tx_offset + sizeof(rndis_data_packet_t));
    usb_memcpy(buffer, buf, buflen);
    g_rndis_class.tx_offset += hdr->MessageLength;
